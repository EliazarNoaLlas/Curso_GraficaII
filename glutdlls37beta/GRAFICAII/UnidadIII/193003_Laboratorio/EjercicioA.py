...
ancho = 300
altura = 200
# Profundidad máxima de trazado de rayos
...
objetos = [
    # Primer objeto
    { 
        'centro': np.array([-0.2, 0, -1]),  # Centro del objeto en coordenadas (x, y, z)
        'radio': 0.7,                      # Radio de la esfera
        'ambiente': np.array([0.1, 0, 0]),  # Componente ambiente del color del objeto (rojo oscuro)
        'difuso': np.array([0.7, 0, 0]),    # Componente difuso del color del objeto (rojo)
        'especular': np.array([1, 1, 1]),  # Componente especular del color del objeto (blanco)
        'brillo': 100,                     # Coeficiente de brillo del objeto
        'reflexion': 0.5                   # Coeficiente de reflexión del objeto
    },
    # Segundo objeto
    {
        'centro': np.array([0.1, -0.3, 0]), # Centro del objeto en coordenadas (x, y, z)
        'radio': 0.1,                      # Radio de la esfera
        'ambiente': np.array([0.1, 0, 0.1]),# Componente ambiente del color del objeto (rojo oscuro)
        'difuso': np.array([0.7, 0, 0.7]),  # Componente difuso del color del objeto (rojo y azul)
        'especular': np.array([1, 1, 1]),  # Componente especular del color del objeto (blanco)
        'brillo': 100,                     # Coeficiente de brillo del objeto
        'reflexion': 0.5                   # Coeficiente de reflexión del objeto
    },
    # Tercer objeto
    {
        'centro': np.array([-0.3, 0, 0]),  # Centro del objeto en coordenadas (x, y, z)
        'radio': 0.15,                     # Radio de la esfera
        'ambiente': np.array([0, 0.1, 0]),  # Componente ambiente del color del objeto (verde oscuro)
        'difuso': np.array([0, 0.6, 0]),    # Componente difuso del color del objeto (verde)
        'especular': np.array([1, 1, 1]),  # Componente especular del color del objeto (blanco)
        'brillo': 100,                     # Coeficiente de brillo del objeto
        'reflexion': 0.5                   # Coeficiente de reflexión del objeto
    },
    # Cuarto objeto
    {
        'centro': np.array([0, -9000, 0]), # Centro del objeto en coordenadas (x, y, z)
        'radio': 9000 - 0.7,               # Radio de la esfera
        'ambiente': np.array([0.1, 0.1, 0.1]), # Componente ambiente del color del objeto (gris oscuro)
        'difuso': np.array([0.6, 0.6, 0.6]),   # Componente difuso del color del objeto (gris)
        'especular': np.array([1, 1, 1]),  # Componente especular del color del objeto (blanco)
        'brillo': 100,                     # Coeficiente de brillo del objeto
        'reflexion': 0.5                   # Coeficiente de reflexión del objeto
    }
